...
# Data cleanup and wrangling is in neon_data_cleanup.R
...
lm3.1 <- lm(np_ratio_foliar~latitude, data=neon_site)
lm3.2 <- lm(np_ratio_foliar~et_mm_avg, data=neon_site)
lm3.3 <- lm(np_ratio_foliar~mat_deg_c, data=neon_site)
lm3.4 <- lm(np_ratio_foliar~map_mm, data=neon_site)
lm3.5 <- lm(np_ratio_foliar~n_percent_soil, data=neon_site)
lm3.6 <- lm(np_ratio_foliar~c_percent_soil, data=neon_site)
lm3.7 <- lm(np_ratio_foliar~netnmin_ugpergperday, data=neon_site)
lm3.8 <- lm(np_ratio_foliar~extractable_p_soil, data=neon_site)
lm3.9 <- lm(np_ratio_foliar~npp_kg_c_avg, data=neon_site)

lm4.1 <- lm(cn_ratio_foliar~latitude, data=neon_site)
lm4.2 <- lm(cn_ratio_foliar~et_mm_avg, data=neon_site)
lm4.3 <- lm(cn_ratio_foliar~mat_deg_c, data=neon_site)
lm4.4 <- lm(cn_ratio_foliar~map_mm, data=neon_site)
lm4.5 <- lm(cn_ratio_foliar~n_percent_soil, data=neon_site)
lm4.6 <- lm(cn_ratio_foliar~c_percent_soil, data=neon_site)
lm4.7 <- lm(cn_ratio_foliar~netnmin_ugpergperday, data=neon_site)
lm4.8 <- lm(cn_ratio_foliar~extractable_p_soil, data=neon_site)
lm4.9 <- lm(cn_ratio_foliar~npp_kg_c_avg, data=neon_site)

lm5.1 <- lm(cp_ratio_foliar~latitude, data=neon_site)
lm5.2 <- lm(cp_ratio_foliar~et_mm_avg, data=neon_site)
lm5.3 <- lm(cp_ratio_foliar~mat_deg_c, data=neon_site)
lm5.4 <- lm(cp_ratio_foliar~map_mm, data=neon_site)
lm5.5 <- lm(cp_ratio_foliar~n_percent_soil, data=neon_site)
lm5.6 <- lm(cp_ratio_foliar~c_percent_soil, data=neon_site)
lm5.7 <- lm(cp_ratio_foliar~netnmin_ugpergperday, data=neon_site)
lm5.8 <- lm(cp_ratio_foliar~extractable_p_soil, data=neon_site)
lm5.9 <- lm(cp_ratio_foliar~npp_kg_c_avg, data=neon_site)
```
...
# Foliar N:P vs. latitude 
lat_np <- ggplot(neon_site, aes(x=latitude, y=np_ratio_foliar)) +
  geom_point() +
  geom_smooth(method="lm") +
...
# Foliar C:N vs latitude
lat_cn <- ggplot(neon_site, aes(x=latitude, y=cn_ratio_foliar)) +
  geom_point() +
...
# Foliar CP ratio vs latitude
lat_cp <- ggplot(neon_site, aes(x=latitude, y=cp_ratio_foliar)) +
  geom_point() +
  geom_smooth(method="lm") +
...
et_np <- ggplot(neon_site, aes(x=et_mm_avg, y=np_ratio_foliar)) +
  geom_point() +
  geom_smooth(method="lm") +
...
et_cn <- ggplot(neon_site, aes(x=et_mm_avg, y=cn_ratio_foliar)) +
  geom_point() +
...
et_cp <- ggplot(neon_site, aes(x=et_mm_avg, y=cp_ratio_foliar)) +
  geom_point() +
  geom_smooth(method="lm") +
  labs(x="", y="Foliar C:P ratio (mass)") +
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")),
...
lat_col <- plot_grid(lat_n, lat_cn, lat_p, lat_cp, lat_np, nrow=5, 
                      align = "v", labels=c("A", "B", "C", "D", "E"))
lat_col_label <- ggdraw(lat_col) +
  draw_label(expression(Latitude~(degree)), color="black", vjust=0, y=0)

# make versions of et figs that have no y axis title (so they can be shared)
et_n2 <- et_n + theme(axis.title.y=element_blank())
et_p2 <- et_p + theme(axis.title.y=element_blank())
et_cn2 <- et_cn + theme(axis.title.y=element_blank())
et_np2 <- et_np + theme(axis.title.y=element_blank())
et_cp2 <- et_cp + theme(axis.title.y=element_blank())

et_col <- plot_grid(et_n2, et_cn2, et_p2, et_cp2, et_np2, nrow=5, 
...
foliarnp_soilnmin <- ggplot(neon_site, aes(x=netnmin_ugpergperday, y=np_ratio_foliar)) +
  geom_point() +
  geom_smooth(method="lm") +
  labs(x=expression("Soil N mineralization rate"~(mu*g~g^-1~day^-1)), 
...
foliarnp_soilp <- ggplot(neon_site, aes(x=extractable_p_soil, y=np_ratio_foliar)) +
  geom_point() +
  geom_smooth(method="lm") +
  labs(x=expression("Soil extractable P"~(mg~P~kg^-1~soil)), 
       y="Foliar N:P ratio (mass)") +
...
## Site variable importance calculations with VIP
...
arrow <- readPNG(here("manuscript", "prelim_figs", "arrow.png")) # need to load magick package
