---
title: "NEON Data Cleanup + Wrangling"
...
source(here("code", "n.def.calc.trans.R"))
...
stackByTable(filepath="filesToStack10026")
...
Altogether, 1,076 foliar chemistry (C, N, and P concentration) samples representing 164 woody and herbaceous plant species at 47 terrestrial sites were accessed from the Plant foliar traits data product (DP1.10026.001). Foliar data come from one collection bout per site conducted between 2016-2019. 
...
# Save CSV
write.csv(foliar_chem_field, here("data", "neon_foliar_chem.csv"), row.names=FALSE)
```
...
``` {r join foliar data to site data}
# calculate site-mean foliar chemistry
foliar_site <- (foliar_chem_all %>%
                  group_by(site_id) %>%
                  summarize(across(where(is.numeric), mean, na.rm=TRUE)))
...
foliar_site_alt <- (foliar_chem_all %>%
                      group_by(site_id) %>%
                      summarize(across(where(is.numeric), mean_sd, na.rm=TRUE)))
foliar_soil_site_alt <- left_join(foliar_site_alt, site_clim_soil,
                                  by="site_id")
write.csv(foliar_soil_site_alt, here("data", "neon_data_site_avg_alt.csv"), row.names=FALSE)
```
